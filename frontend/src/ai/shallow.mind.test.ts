...
describe("ShallowMind", () => {
  ...
  beforeEach(() => cardsHelper.initialize());
    ...
    describe("when there are no dominating cards", () => {
      describe("then return empty", () => {
        it.each(
          ...
          const handCards = toCards(handCardsStr);
          const mind = new ShallowMind(handCards);
          expect(mind.followSuit(cardsSet).length).toBe(0);
        });
      });
    });
  });
      ...
      describe("then return cards", () => {
        it.each(
          [
            ...
            expected: b as string,
            handCardsStr: a as string,
          })),
        )("$handCardsStr => $expected", ({ handCardsStr, expected }) => {
          const handCards = toCards(handCardsStr);
          const mind = new ShallowMind(handCards);
          expect(
            mind
