...
package demo.ddz.setup.argument.messaging;
...
import demo.ddz.setup.argument.AnnotationFinder;
    ...
    class WhenAdaptorIsNull {
      ...
      void thenReturn() {
        try (MockedStatic<ReactiveSecurityContextHolder> holder =
          ...
          PlayerIdArgumentResolver resolver = new PlayerIdArgumentResolver();
          MethodParameter parameter = mock(MethodParameter.class);
          StepVerifier.create(resolver.resolveArgument(parameter, mock(ErrorMessage.class)))
              .expectNext(UID_1)
              .verifyComplete();
        }
      }
